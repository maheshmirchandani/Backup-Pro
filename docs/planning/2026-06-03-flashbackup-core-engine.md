...
- **Task 41:** `cmd/flashbackup/help.go` `help <subcommand>` verb-form (sources from a constants-table `subcommandHelpTexts`) AND keeps each subcommand's existing `--help` via the handler's own `fs.Usage` block. Future refactor may have the existing handlers pull from the same table; for v0.1 the table is read by `runHelp` only. Per Tech Writer hat. Amended 2026-06-05 per Task 41 review A1.
