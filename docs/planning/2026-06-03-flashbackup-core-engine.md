...
- **Task 37:** `cmd/flashbackup/backup.go` move-mode confirmation. UIRenderer emits `UIEvent{Kind:UIEvtPrompt,Path:"DELETE"}`; cmd reads literal `DELETE\n` from stdin; case-sensitive exact match. AC-7, AC-8.
